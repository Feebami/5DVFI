...
class UNet3d(nn.Module):
    def __init__(self, n_steps, emb_dim, channel_mult, attn_heads):
        ...
        self.down1 = Block(self.channels[0], self.channels[1], (1,2,2), emb_dim)
        self.down11 = Block(self.channels[1], self.channels[1], 1, emb_dim)
        # self.down_attn1 = SelfAttention(self.channels[1], self.attn_heads) if attn_heads > 0 else nn.Identity()
        self.down2 = Block(self.channels[1], self.channels[2], (1,2,2), emb_dim)
        self.down22 = Block(self.channels[2], self.channels[2], 1, emb_dim)
        # self.down_attn2 = SelfAttention(self.channels[2], self.attn_heads) if attn_heads > 0 else nn.Identity()
        self.down3 = Block(self.channels[2], self.channels[3], (1,2,2), emb_dim)
        self.down33 = Block(self.channels[3], self.channels[3], 1, emb_dim)
        # self.down_attn3 = SelfAttention(self.channels[3], self.attn_heads) if attn_heads > 0 else nn.Identity()
        self.down4 = Block(self.channels[3], self.channels[3], (1,2,2), emb_dim)
        self.down44 = Block(self.channels[3], self.channels[3], 1, emb_dim)
    ...
    def forward(self, x, t):
        ...
        # x2 = F.max_pool3d(x1, (1, 2, 2))
        x2 = self.down1(x1, t)
        x2 = self.down11(x2, t)
        # x2 = self.down_attn1(x2)
        
        # x3 = F.max_pool3d(x2, (1, 2, 2))
        x3 = self.down2(x2, t)
        x3 = self.down22(x3, t)
        # x3 = self.down_attn2(x3)
        
        # x4 = F.max_pool3d(x3, (1, 2, 2))
        x4 = self.down3(x3, t)
        x4 = self.down33(x4, t)
        # x4 = self.down_attn3(x4)
        
        # x5 = F.max_pool3d(x4, (1, 2, 2))
        x5 = self.down4(x4, t)
        x5 = self.down44(x5, t)
